"""Document post-processing — operations applied to the DOCX output after rendering."""
...
from md2docx.content import _RT_MARKER
...
def apply(docx_path: Path, template: Path, style_map: StyleMap, partials: dict[str, Path]) -> None:
    """Apply all post-processing steps to the rendered DOCX."""
    strip_bookmarks(docx_path)
    sync_headers(docx_path, template)
    sync_fonts(docx_path, template)
    if style_map.right_tab:
        apply_right_tab_stops(docx_path, style_map.right_tab)
    apply_run_styles(docx_path, style_map.run_styles)
    splice_partials(docx_path, partials)
...
def strip_bookmarks(docx_path: Path) -> None:
    ...
    _write_zip(docx_path, names, files)
...
def apply_run_styles(docx_path: Path, style_map: dict[str, RunStyle]) -> None:
    """Apply template run styles to bold, italic, code, and bullet runs."""
...
def apply_right_tab_stops(docx_path: Path, tab_stop: dict) -> None:
    """Split runs at _RT_MARKER, insert <w:tab/>, and apply tab stop definition to pPr."""
    with zipfile.ZipFile(docx_path, 'r') as zin:
        names = zin.namelist()
        files = {name: zin.read(name) for name in names}
...
def splice_partials(docx_path: Path, partials: dict[str, Path]) -> None:
    ...
    for name in sorted(unclaimed):
        warnings.warn(f"placeholder {{{{{name}}}}} has no matching partial", stacklevel=2)
...
def _merge_numbering(out_files: dict, p_files: dict, p_num_ids: set) -> dict[str, str]:
    ...
    if out_num_bytes:
        ...
        out_num_root = ET.fromstring(f'<w:numbering xmlns:w="{W}"/>')
